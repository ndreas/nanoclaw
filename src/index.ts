...
async function runAgent(
  ...
  try {
    ...
    if (output.status === 'error') {
      // Detect stale/corrupt session via specific error patterns — clear immediately.
      ...
      // Track consecutive failures and clear corrupted session after threshold
      ...
      if (!isStaleSession && failureCount >= MAX_SESSION_FAILURES) {
        logger.warn(
          {
            group: group.name,
            failureCount,
